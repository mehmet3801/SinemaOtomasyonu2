...
namespace SinemaOtomasyonu2
{
    public partial class Seansekle : Form
    {
        ...
        {
            bag.Open();
            SqlCommand komut = new SqlCommand("SELECT * FROM seans_bilgileri WHERE salonadi=@salonadi AND tarih=@tarih", bag);
            komut.Parameters.AddWithValue("@salonadi", comboBox2.Text);
            komut.Parameters.AddWithValue("@tarih", dateTimePicker1.Value.Date); // Doğrudan DateTime olarak ekliyoruz.
            SqlDataReader read = komut.ExecuteReader();
            while (read.Read() == true)
            {
